...
async function loadPoiDetail() {
  ...
  try {
    ...
    const response = await fetch(`https://quanhne.id.vn/api/poi/${poiId}`);
...
function showPoiDetail(poi) {
  ...
  document.getElementById("poiType").textContent =
    poi.typename || "Chưa phân loại";
  document.getElementById("poiAddress").textContent =
    poi.address || "Chưa có địa chỉ";
  document.getElementById("poiOpenTime").textContent =
    poi.openTime || "Chưa có thông tin";
  document.getElementById("poiCloseTime").textContent =
    poi.closeTime || "Chưa có thông tin";
  document.getElementById("poiDescription").textContent =
