    ...
    document.getElementById("tourInfo").innerHTML = `
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <small class="text-muted">${tour.duration} ngày - ${
      tour.address || "Không có địa chỉ"
    }</small>
                </div>
            </div>
        `;
...
function renderAvailablePois(filteredPois = null) {
  ...
  container.innerHTML = poisToRender
    .map(
      (poi) => `
...
function renderSelectedPois() {
  ...
  if (selectedPois.length === 0) {
    container.innerHTML =
      '<p class="text-muted text-center">Chưa có POI nào được chọn</p>';
    return;
  }

  container.innerHTML = selectedPois
    .map(
      (poi) => `
        <div class="selected-poi-item">
            ...
            <div class="btn-group">
                <button class="btn btn-sm btn-outline-secondary" onclick="movePoiUp(${
                  poi.poiId
                })" 
                        ${poi.orderintour === 1 ? "disabled" : ""}>
                    <i class="fas fa-arrow-up"></i>
                </button>
                <button class="btn btn-sm btn-outline-secondary" onclick="movePoiDown(${
                  poi.poiId
                })"
                        ${
                          poi.orderintour === selectedPois.length
                            ? "disabled"
                            : ""
                        }>
                    <i class="fas fa-arrow-down"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger" onclick="togglePoiSelection(${
                  poi.poiId
                })">
...
function movePoiUp(poiId) {
  ...
  if (index > 0) {
    [selectedPois[index], selectedPois[index - 1]] = [
      selectedPois[index - 1],
      selectedPois[index],
    ];
...
function setupSearchFilter() {
  document.getElementById("searchPoi").addEventListener("input", function () {
    ...
    const filtered = availablePois.filter(
      (poi) =>
...
async function saveTourPois() {
  ...
  try {
    const response = await fetch("https://quanhne.id.vn/api/assignpoi", {
      method: "POST",
      headers: {
