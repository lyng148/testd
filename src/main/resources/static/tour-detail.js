...
async function loadPOIs() {
  ...
  try {
    ...
    const response = await fetch(
      `https://quanhne.id.vn/api/listtour/${tourId}`
    );
      ...
      (poi, index) => `
        <div class="d-flex align-items-center p-3 border-bottom">
            ...
            <div class="action-buttons">
                <button class="btn btn-sm btn-outline-primary me-2" onclick="viewDetails(${
                  poi.id
                })">
...
async function deletePoi(index) {
  ...
  try {
    const response = await fetch(
      `https://quanhne.id.vn/api/poioftour/${poi.tourPoiId}`,
      {
        method: "DELETE",
      }
    );
