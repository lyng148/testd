...
// API Base URL
const API_BASE_URL = "https://quanhne.id.vn";
...
function initializeFilters() {
  ...
  document
    ...
    .addEventListener("change", applySortingAndDisplay);

  // Filter inputs
  document
    .getElementById("typetour")
    .addEventListener("change", updateSearchButtonState);
  document
    .getElementById("address")
    .addEventListener("input", updateSearchButtonState);
  document
...
function updateSearchButtonState() {
  ...
  if (hasFilters) {
    ...
    searchBtn.innerHTML =
      '<i class="fas fa-search me-2"></i>Tìm kiếm với bộ lọc';
  } else {
...
async function performSearch() {
  ...
  try {
    ...
    if (newSearchParams.typetour)
      params.append("typetour", newSearchParams.typetour);
    if (newSearchParams.address)
      params.append("address", newSearchParams.address);
    ...
    const apiUrl = `${API_BASE_URL}/api/tour${
...
function displayTours(tours) {
  ...
  list.innerHTML = uniqueTours
    .map((tour, index) => createTourItem(tour, index))
    .join("");
}
                    ...
                    <div class="tour-item-content">
                        ...
                        <div class="tour-item-footer">
                            <div class="tour-item-price">
                                <span class="price-amount-large">${formatPrice(
                                  tour.price
                                )}</span>
                                <span class="price-label-large">/ người</span>
                            </div>
                            <div class="tour-action-buttons">
                                <button class="btn btn-detail" onclick="viewTourDetail(${
                                  tour.id
                                })" title="Xem chi tiết tour ${tour.name}">
                                    <i class="fas fa-eye me-1"></i>Chi tiết
                                </button>
                                <button class="btn btn-edit" onclick="editTour(${
                                  tour.id
                                })" title="Chỉnh sửa tour ${tour.name}">
                                    <i class="fas fa-edit me-1"></i>Sửa
                                </button>
                                <button class="btn btn-delete" onclick="deleteTour(${
                                  tour.id
...
function deleteTour(id) {
  ...
  fetch(`https://quanhne.id.vn/api/tour/${id}`, {
    method: "DELETE",
  })
    .then((response) => {
      if (response.ok) {
